...
#
def setup_laststep_pred_data(csv_file="laststep_pred.v1.csv.gz", strip_cmdhistory=False, cmds_only=False):
...
def setup_state_dict_data(csv_file="laststep_pred.v1.csv.gz", use_tfidf=True):
    """
    Sets up the numeric X,Y arrays for command history standard classification methods experiment.
    """
    train_df, test_df = setup_laststep_pred_data(csv_file=csv_file)
    Y_train = np.zeros(len(train_df))
    Y_train[train_df.target_text == 'pos'] = 1
    Y_test = np.zeros(len(test_df))
    Y_test[test_df.target_text == 'pos'] = 1
