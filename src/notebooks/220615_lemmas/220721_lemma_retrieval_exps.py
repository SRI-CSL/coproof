...
for expname, theory_bank in experiments:
    ...
    for svm_name, svc in SVMs:
        ...
        # df.to_csv("svc_results.mult.{}.csv".format(suffix), header=True, index=False)
        df.to_csv("svc_results.l1.{}.csv".format(suffix), header=True, index=False)        
        print("{}/{}: train_acc={:.3f}, test_acc={:.3f}".format(expname, svm_name, train_acc, test_acc))
...
df = pd.DataFrame(svc_results)
df.to_csv("svc_results.csv", header=True, index=False)
