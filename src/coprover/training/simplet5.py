"""
Heavily modified variant of SimpleT5
"""
...
class SimpleT5:
    ...
    def from_pretrained(self, model_type="t5", model_name="t5-base",
                        tokenizer=None) -> None:
        """
        loads T5/MT5 existing English language model for training/finetuning
        Args:
            model_type (str, optional): "t5" or "mt5" . Defaults to "t5".
            model_name (str, optional): exact model architecture name, "t5-base" or "t5-large". Defaults to "t5-base".
        """
        if model_type == "t5":
            # TODO: Construct a SentencePiece vocabulary over the selected training data
    ...
    # TODO: Stub that uses train() with dataframes,
    # another that uses train() with iterable datasets
    ...
    def train(
        ...
        save_only_last_epoch: bool = False,
        num_gpus = 1,  # Set to number of GPUs availabe on system.  Use 0 for CPU only.
        accelerator="ddp" # Including accelerator strategy to use
    ):
        """
        ...
        # Set the logger, otherwise default to Tensorboard
        ...
        elif accelerator == "ddp":
            # DDP prefers setting find_unused_parameters to false for performance
            trainer = pl.Trainer(
                logger=loggers,
                callbacks=callbacks,
                max_epochs=max_epochs,
    ...
    def load_model(self, model_dir,
        ...
        """
        print("Loading model_type={}, dir={}, use_gpu={}".format(model_type,
                                                                 model_dir,
                                                                 use_gpu))
        if not(Path(model_dir).exists()):
            raise Exception("Given model_dir={} does not exist!".format(model_dir))
    ...
    def predict(
        self,
        source_text: str,
        max_length = None,  # If activated, use this as the limit
        ...
        # If the source text exceeds the set limit, then truncate to the first N
        if self.source_max_token_len is not None:
            input_ids = input_ids[:, 0:self.source_max_token_len] # Reshape to truncate just the first N tokens
        input_ids = input_ids.to(self.device)
        if max_length is None:            
            max_length = self.target_max_token_len
        if max_length > self.target_max_token_len:
                raise Exception("Requested max length={} > trained max length={}".format(
                    max_length, self.target_max_token_len))
