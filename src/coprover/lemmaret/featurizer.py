"""
Featurizes the lemma or sequent into the seq2seq input form.

Note this is similar to the command prediction featurization, but includes a stab
at including type information.  This is currently restricted to primitive types, and
predicate or composite data types are rendered to generic forms.
"""

import json

from coprover.feats.featurize_cmdpred import ANT_TOK, CONS_TOK
...
def process_formula_json(elt, parent_key=None, type_lookup={}, 
                         ret_str=False, process_types=True):
    """
    Main entry point from JSON expression to token stream for input.
    ...
    if isinstance(elt, list):
        ...
        if len(elt) == 2 and elt[0] == 'tag' and elt[1] == 'tuple':
            return [None]
        return [process_formula_json(x, type_lookup=type_lookup) for x in elt]
        ...
        if TAG in elt:
            ...
            if elt_tag == CONSTANT and parent_key == OPERATOR:
                accum.extend(process_formula_json(safe_get(elt, CONSTANT_NAME, ID), type_lookup=type_lookup))
            for k, v in elt.items():
                if k not in TABU_TAGS:
                    accum.append(k)
                    accum.extend(process_formula_json(v, parent_key=k, type_lookup=type_lookup))
            if VARIABLE_NAME in elt:
                # Process variables
                if not(process_types):
                    accum.append("UnkType")
                elif TYPE in elt:
        ...
        if module[TAG] == THEORY:
            ...
            for decl in module[DECLARATIONS]:
                ...
                if decl[TAG] == CONST_DECL and \
                    ...
                    expression = process_formula_json(decl[CONST_DEF][EXPRESSION], type_lookup=type_lookup)
...
def read_proof_session(json_root):
    ...
    if proof_seq is not None:
        ...
        for idx in range(0, len(proof_seq), 2):
            ...
            state_toks = bfs_flatten(process_formula_json(proof_step, type_lookup=type_lookup))
